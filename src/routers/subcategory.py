...
@router.post("/create/", response_model=SubCategoryInfo)
...
@router.get("/all/", response_model=list[SubCategoryInfo])
def get_all_subcategories(db: Session = Depends(get_db)):
    return SubCategoryService.get_all_subcategories(db=db)


@router.get("/detail/{subcategory_id}/", response_model=SubCategoryInfo)
...
@router.put("/update/{subcategory_id}/", response_model=SubCategoryInfo)
